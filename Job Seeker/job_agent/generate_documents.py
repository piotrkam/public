...
resume_doc = Document()
resume_doc.margins = Inches(0.75)

# Header
name_para = resume_doc.add_paragraph()
name_para.alignment = WD_ALIGN_PARAGRAPH.CENTER
name_run = name_para.add_run('PIOTR KAMINSKI')
name_run.font.size = Pt(16)
name_run.font.bold = True

contact_para = resume_doc.add_paragraph()
contact_para.alignment = WD_ALIGN_PARAGRAPH.CENTER
contact_text = contact_para.add_run('piotrkaminski1@protonmail.com | +48 510 934 702 | Krakow, Poland | LinkedIn')
contact_text.font.size = Pt(10)

resume_doc.add_paragraph()

# PROFESSIONAL SUMMARY
summary_heading = resume_doc.add_heading('PROFESSIONAL SUMMARY', level=2)
summary_text = resume_doc.add_paragraph(
...
exp_heading = resume_doc.add_heading('PROFESSIONAL EXPERIENCE', level=2)

# Job 1
job1_title = resume_doc.add_paragraph()
run = job1_title.add_run('Lead Solutions Engineer')
run.bold = True
run = job1_title.add_run(' | ')
run = job1_title.add_run('Luigi\'s Box')
run.bold = True
run = job1_title.add_run(' | ')
run = job1_title.add_run('Jan 2025 – Present')
run.italic = True

job1_desc = resume_doc.add_paragraph(
    'Led onboarding and integration initiatives for key accounts, achieving zero churn rate from implementation delivery. '
    'Redesigned onboarding processes to improve efficiency and client success outcomes. Managed Tier 1/2 technical delivery '
    'across multiple concurrent projects; coordinated cross-functional teams (engineering, product, support) to ensure seamless '
    'project execution. Implemented GenAI/AI-powered solutions including search optimization and recommender systems to enhance '
    'product capabilities. Mentored junior team members on technical delivery best practices and customer-centric approaches.'
)
job1_desc.paragraph_format.line_spacing = 1.15

# Job 2
job2_title = resume_doc.add_paragraph()
run = job2_title.add_run('Technical Account Manager')
run.bold = True
run = job2_title.add_run(' | ')
run = job2_title.add_run('Flexiroam')
run.bold = True
run = job2_title.add_run(' | ')
run = job2_title.add_run('Jan 2023 – Dec 2024')
run.italic = True

job2_desc = resume_doc.add_paragraph(
    'Coordinated 100% implementation success rate across all B2B platform migrations (50+ accounts) with zero integration failures. '
    'Drove 25% YoY account growth through effective client relationship management and proactive project delivery oversight. '
    'Led complex B2B platform migration affecting 50+ enterprise accounts with custom requirements; managed technical dependencies, '
    'timeline coordination, and stakeholder communication. Developed and maintained REST API integrations for third-party systems; '
    'provided ongoing technical mentoring to client engineering teams and internal support staff.'
)
job2_desc.paragraph_format.line_spacing = 1.15

# Job 3
job3_title = resume_doc.add_paragraph()
run = job3_title.add_run('Technical Support Engineer')
run.bold = True
run = job3_title.add_run(' | ')
run = job3_title.add_run('Rapid')
run.bold = True
run = job3_title.add_run(' | ')
run = job3_title.add_run('Jan 2020 – Dec 2022')
run.italic = True

job3_desc = resume_doc.add_paragraph(
    ...
    'Optimized support workflows and processes, improving team efficiency by 10–15% and reducing mean-time-to-resolution by 20%. '
    'Achieved 89% first-contact resolution satisfaction rating through process standardization and team skill development. '
...
job3_desc.paragraph_format.line_spacing = 1.15

# SKILLS
skills_heading = resume_doc.add_heading('CORE COMPETENCIES', level=2)
skills_categories = [
...
for category, skills in skills_categories:
    skill_para = resume_doc.add_paragraph(style='List Bullet')
    run = skill_para.add_run(category + ': ')
    run.bold = True
    skill_para.add_run(skills)
    skill_para.paragraph_format.line_spacing = 1.15

# LANGUAGES
lang_heading = resume_doc.add_heading('LANGUAGES', level=2)
lang_para = resume_doc.add_paragraph(style='List Bullet')
lang_para.add_run('English – C1/C2 (Professional fluency)')
lang_para = resume_doc.add_paragraph(style='List Bullet')
lang_para.add_run('Polish – Native')

# Save
resume_doc.save('data/resumes/cv_JJ_010.docx')
print('Resume created: data/resumes/cv_JJ_010.docx')
...
letter_doc = Document()
letter_doc.margins = Inches(0.75)

# Header
header_para = letter_doc.add_paragraph()
...
letter_doc.add_paragraph()

# Date and recipient
date_para = letter_doc.add_paragraph('17 April 2026')
date_para.space_after = Pt(12)

recipient_para = letter_doc.add_paragraph('Hiring Manager\nChaos Gears')
recipient_para.space_after = Pt(12)

# Salutation
letter_doc.add_paragraph('Dear Hiring Manager,')

# Opening paragraph
opening = letter_doc.add_paragraph(
...
body1 = letter_doc.add_paragraph(
...
body2 = letter_doc.add_paragraph(
...
# Closing paragraph
closing = letter_doc.add_paragraph(
...
# Sign-off
letter_doc.add_paragraph('Best regards,')
letter_doc.add_paragraph()
letter_doc.add_paragraph('Piotr Kaminski')

# Save
letter_doc.save('data/cover_letters/letter_JJ_010.docx')
print('Cover letter created: data/cover_letters/letter_JJ_010.docx')

# ============================================================
# 3. CREATE APPLICATION NOTES (JSON)
# ============================================================
...
with open('data/analyses/notes_JJ_010.json', 'w') as f:
    json.dump(notes, f, indent=2)
print('Application notes created: data/analyses/notes_JJ_010.json')
